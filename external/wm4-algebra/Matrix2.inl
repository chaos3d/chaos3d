...
{
    m_afEntry[0] = cos(fAngle);
    m_afEntry[2] = sin(fAngle);
...
void Matrix2<Real>::ToAngle (Real& rfAngle) const
{
    ...
    rfAngle = atan2(m_afEntry[2],m_afEntry[0]);
}
//----------------------------------------------------------------------------
template <class Real>
...
void Matrix2<Real>::EigenDecomposition (Matrix2& rkRot, Matrix2& rkDiag) const
{
    ...
    Real fTmp = (Real)1.0/sqrt(fCos*fCos + fSin*fSin);
    fCos *= fTmp;
    fSin *= fTmp;
