...
public:
	...
	// from euler angle (X,Y,Z)
	Quaternion (Real fX, Real fY, Real fZ);
	...
	// conversion between quaternions and XYZ euler angle
	Quaternion& FromEulerAngle (Real fX, Real fY, Real fZ);
	void ToEulerAngle (Real& fX, Real& fY, Real& fZ) const;
	...
	public:
		...
		void SetAngles (Real fMinAngle, Real fMaxAngle)
		{
			...
			m_fCosMinAngle = cos(m_fMinAngle);
			m_fSinMinAngle = sin(m_fMinAngle);
			m_fCosMaxAngle = cos(m_fMaxAngle);
			m_fSinMaxAngle = sin(m_fMaxAngle);
			...
			m_fCosAvrAngle = cos(fAvrAngle);
			m_fSinAvrAngle = sin(fAvrAngle);
		}
...
private:
	...
	static Real ms_fRootTwo;
	static Real ms_fRootHalf;
