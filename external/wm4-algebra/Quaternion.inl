...
//----------------------------------------------------------------------------
template <class Real>
Quaternion<Real>::Quaternion (Real fX, Real fY, Real fZ)
{
    FromEulerAngle(fX,fY,fZ);
}
//----------------------------------------------------------------------------
template <class Real>
    ...
    const Matrix3<Real>& rkRot)
{
    ...
    else
    {
        ...
        fRoot = sqrt(rkRot(i,i)-rkRot(j,j)-rkRot(k,k)+(Real)1.0);
    ...
    const
{
    ...
    if (fSqrLength > 1e-06)
    {
        rfAngle = ((Real)2.0)*acos(m_afTuple[0]);
        Real fInvLength = (Real)1.0/sqrt(fSqrLength);
...
//----------------------------------------------------------------------------
template <class Real>
Quaternion<Real>& Quaternion<Real>::FromEulerAngle ( Real fX, Real fY, Real fZ)
{
    // http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles

	Real halfX = Real(.5) * fX, halfY = Real(.5) * fY, halfZ = Real(.5) * fZ;
    Real fSinX = sin(halfX), fCosX = cos(halfX);
    Real fSinY = sin(halfY), fCosY = cos(halfY);
    Real fSinZ = sin(halfZ), fCosZ = cos(halfZ);
    m_afTuple[0] = fCosX * fCosY * fCosZ + fSinX * fSinY * fSinZ;
    m_afTuple[1] = fSinX * fCosY * fCosZ - fCosX * fSinY * fSinZ;
    m_afTuple[2] = fCosX * fSinY * fCosZ + fSinX * fCosY * fSinZ;
    m_afTuple[3] = fCosX * fCosY * fSinZ - fSinX * fSinY * fCosZ;

    return *this;
}
//----------------------------------------------------------------------------
template <class Real>
void Quaternion<Real>::ToEulerAngle (Real& fX, Real& fY, Real& fZ)
    const
{
    // http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
    ...
    if (fSqrLength > 1e-06)
    {
        fX = atan2( Real(2)*(m_afTuple[0]*m_afTuple[1] + m_afTuple[2]*m_afTuple[3]),
			Real(1) - Real(2)*(m_afTuple[1]*m_afTuple[1] + m_afTuple[2]*m_afTuple[2]));
		fY = asin( Real(2)*(m_afTuple[0]*m_afTuple[2] - m_afTuple[1]*m_afTuple[3]) );
		fZ = atan2( Real(2)*(m_afTuple[0]*m_afTuple[3] + m_afTuple[1]*m_afTuple[2]),
			Real(1) - Real(2)*(m_afTuple[2]*m_afTuple[2] + m_afTuple[3]*m_afTuple[3]));
    }
    else
    {
        fX = (Real)0.0;
...
Quaternion<Real> Quaternion<Real>::Exp () const
{
    ...
    Real fAngle = sqrt(m_afTuple[1]*m_afTuple[1] +
        m_afTuple[2]*m_afTuple[2] + m_afTuple[3]*m_afTuple[3]);

    Real fSin = sin(fAngle);
    kResult.m_afTuple[0] = cos(fAngle);
...
Quaternion<Real> Quaternion<Real>::Log () const
{
    ...
    if (fabs(m_afTuple[0]) < (Real)1.0)
    {
        Real fAngle = cos(m_afTuple[0]);
        Real fSin = sin(fAngle);
        if (fabs(fSin) >= 1e-06)
        {
    ...
    const Quaternion& rkQ)
{
    ...
    if (fabs(fAngle) >= 1e-06)
    {
        Real fSin = sin(fAngle);
        Real fInvSin = ((Real)1.0)/fSin;
        Real fTAngle = fT*fAngle;
        Real fCoeff0 = sin(fAngle - fTAngle)*fInvSin;
        Real fCoeff1 = sin(fTAngle)*fInvSin;
    ...
    const Quaternion& rkP, const Quaternion& rkQ, int iExtraSpins)
{
    ...
    {
        Real fSin = sin(fAngle);
        Real fPhase = (Real)M_PI*iExtraSpins*fT;
        Real fInvSin = ((Real)1.0)/fSin;
        Real fCoeff0 = sin(((Real)1.0-fT)*fAngle-fPhase)*fInvSin;
    ...
    const Vector3<Real>& rkV2)
{
    ...
    {
        Real fInvLength;
        if (fabs(rkV1[0]) >= fabs(rkV1[1]))
        {
            // V1.x or V1.z is the largest magnitude component
            fInvLength = (Real)1.0/sqrt(rkV1[0]*rkV1[0] +
                rkV1[2]*rkV1[2]);
        ...
        else
        {
            ...
            fInvLength = (Real)1.0/sqrt(rkV1[1]*rkV1[1] +
                rkV1[2]*rkV1[2]);
...
Quaternion<Real> Quaternion<Real>::GetClosestXY () const
{
    ...
    if(fabs(fDet) < (Real)0.5 - ms_fTolerance)
    {
        Real fDiscr = (Real)1 - ((Real)4)*fDet*fDet;
        fDiscr = sqrt(fabs(fDiscr));
        ...
        fInvLength = (Real)1.0/sqrt(fC0*fC0 + fS0*fS0);
        fC0 *= fInvLength;
        fS0 *= fInvLength;

        fC1 = m_afTuple[0]*fC0 + m_afTuple[1]*fS0;
        fS1 = m_afTuple[2]*fC0 + m_afTuple[3]*fS0;
        fInvLength = (Real)1.0/sqrt(fC1*fC1 + fS1*fS1);
        fC1 *= fInvLength;
        fS1 *= fInvLength;
    ...
    {
        Real fInvLength = (Real)1.0/sqrt(fabs(fDet));
    ...
    const Constraints& rkYCon) const
{
    ...
    else
    {
        ...
        if (fDet > (Real)0)
        {
            ...
            fAngle = atan2(kTmp[1],kTmp[0]);
            if (fAngle < fMinAngle || fAngle > fMaxAngle)
            {
                fAngle -= (Real)
                    (kTmp[1] >= (Real)0 ? M_PI : -M_PI);
    ...
    const Constraints& rkXCon) const
{
    ...
    if (fabs(fDet) < (Real)0.5 - ms_fTolerance)
    {
        ...
        fInvLength = (Real)1.0/sqrt(fC2*fC2 + fS2*fS2);
        fC2 *= fInvLength;
        fS2 *= fInvLength;
    ...
    const Constraints& rkYCon) const
{
    ...
    else
    {
        ...
        if (fDet < (Real)0)
        {
            ...
            fAngle = atan2(kTmp[2],kTmp[0]);
            if (fAngle < fMinAngle || fAngle > fMaxAngle)
            {
                fAngle -= (Real)
                    (kTmp[2] >= (Real)0 ? M_PI : -M_PI);
                // assert(fMinAngle <= fAngle && fAngle <= fMaxAngle);
            }
