...
static void PNGAPI user_read_data_fcn(png_structp png_ptr, png_bytep data, png_size_t length)
{
	...
	DataStream* ds = (DataStream*)png_ptr->io_ptr;
...
void TexturePNGLoader::unload( Resource* res ){
	// just delete it
	((Texture*)res)->release();
}

Resource* TexturePNGLoader::load( DataStream* ds, char const* ){
	
	if( ds == 0 ){
		LOG_STAT( ERROR, "Data stream is nil." );
		return 0;
	}

	Render2DManager* rm = Render2DManager::getInstance();
	int format = Texture::Format2D_RGBA8888;
	png_byte buffer[8];
